...
public class ProjectController {
	...
	/**通过此方法返回编辑页面*/
	 ...
	 /**实现数据的写入操作
	  * @param entity 用于封装页面中
	  * 表单数据(name,code,....)
	  * 客户端:name="渤海游"&code="tt-..."
	  * SpringMVC:从参数中获取名字和值,然后
	  * 底层会通过反射将这值通过对象
	  * 的set方法注入到具体的参数对象
     ...
     * 问题:控制层的方法中接收页面数据时,
     * 是如何实现的?
     * 1)HttpServletRequest:request.getParameter("参数名")
     * 2)直接通过变量(名字必须与页面参数名相同)
     * 3)通过实体对象(参数必须与对象用对应的set方法一致)
     * 4)...
     * */
